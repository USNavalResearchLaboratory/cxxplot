...
template< typename T > // TODO: This can be simplified
  concept Element_Has_Real_value_type = requires( T c ) { { typename std::decay<decltype( *(c.begin( )))>::type{}  }->std::convertible_to< double >;};
...
// TODO: Should we worry how x(), y() is returned like we do here, or just have a double get_x... only?
...
#define cxxplot_DEFINE_CONCEPT_MEMBER_FUNC_RETURN_REFERENCE_CONVERTIBLE_TO_DOUBLE(fname)\
  template< typename T > \
  concept fname##ReturnReferenceConvertibleToDouble = requires( T v ) { {v.fname( )} ->ReferenceConvertibleToDouble<>; };

#define cxxplot_DEFINE_CONCEPT_MEMBER_FUNC_RETURN_NOT_REFERENCE_CONVERTIBLE_TO_DOUBLE(fname)\
  template< typename T > \
  concept fname##ReturnNotReferenceConvertibleToDouble = requires( T v ) { {v.fname( )} ->NotReferenceConvertibleToDouble<>; };
...
#define cxxplot_DEFINE_CONCEPTS_AND_GET_FUNCTIONS_FOR_MEMBER_FUNC_ACCESS(fname)\
  ...
  cxxplot_DEFINE_CONCEPT_MEMBER_FUNC_RETURN_NOT_REFERENCE_CONVERTIBLE_TO_DOUBLE(fname)\
  template <fname##ReturnReferenceConvertibleToDouble V>    const auto &get_##fname( const V& vec ) { return vec.fname();}\
  template <fname##ReturnReferenceConvertibleToDouble V>    const auto &get_##fname(       V& vec ) { return vec.fname();}\
  template <fname##ReturnNotReferenceConvertibleToDouble V> double      get_##fname( const V& vec ) { return vec.fname();}\
  template <fname##ReturnNotReferenceConvertibleToDouble V> double      get_##fname(       V& vec ) { return vec.fname();}
...
#define cxxplot_DEFINE_CONCEPT_MEMBER_VAR_CONVERTIBLE_TO_BUT_NOT_DOUBLE(vname)\
  template< typename T > \
  concept vname##MemberVarConvertibleToButNotDouble = requires( T v ) { {v.vname} ->ConvertibleToButNotSame<double>; };

#define cxxplot_DEFINE_CONCEPT_MEMBER_VAR_IS_DOUBLE(vname)\
  template< typename T > \
  concept vname##MemberVarIsDouble = requires( T v ) { {v.vname} ->std::same_as<double>; };
...
#define cxxplot_DEFINE_CONCEPTS_AND_GET_FUNCTIONS_FOR_MEMBER_VAR_ACCESS(vname)\
  ...
  cxxplot_DEFINE_CONCEPT_MEMBER_VAR_IS_DOUBLE(vname)\
  template <vname##MemberVarConvertibleToButNotDouble V>       double  get_##vname( const V& vec ) { return vec.vname; } \
  template <vname##MemberVarConvertibleToButNotDouble V>       double  get_##vname(       V& vec ) { return vec.vname; } \
  template <vname##MemberVarIsDouble V>                  const double &get_##vname( const V& vec ) { return vec.vname; }
...
{
  { get_x( a ) } -> std::convertible_to<double>;
  { get_y( a ) } -> std::convertible_to<double>;
};

template< class C >
concept ElementHas_get_x_get_y = requires( C c )
{
  requires Has_get_x_get_y< decltype( *(c.begin( )) ) >;
};
